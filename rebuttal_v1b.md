...
To evaluate downstream relevance, we ran 15,600 generations: thirteen models × 50 shared FineWeb contexts × two temperatures × twelve decoding conditions. Forced-path WCS always uses the untouched FineWeb prefix, making the human target the literal next word for every checkpoint. For open generation, Base models receive raw continuation prompts, while post-trained models use their native chat templates and an explicit continuation instruction. Generation stops normally at EOS, and TTR/MTLD are evaluated over a fixed 100-word window with completion reported separately. Our primary repeated-measures analysis pools the 13,593 generations reaching the fixed window and controls for both the shared context and the exact model × temperature × sampler × parameter configuration. Reachable target paths are associated with +0.0089 TTR (95% CI [0.0029, 0.0149]) and +3.47 MTLD (95% CI [0.27, 6.66]). Per-model configuration plots are reported descriptively at [URL1_WCS_DIVERSITY_CORRELATION].
...
For the full WCS audit, we report 95% percentile confidence intervals from 2,000 target-word cluster-bootstrap draws, preserving the dependence among the 50 contexts belonging to each word. For downstream validation, the pooled estimates use sandwich uncertainty clustered over the 50 shared contexts, accounting for their repeated use across models and decoding configurations.
